...
namespace al3d
{
    namespace Rendering
    {
        ...
        void SoftwareRenderer::workerPrepareFacesLoop(unsigned refIdFaces, unsigned refIdNormals) {
            ...
            for (unsigned i = 0; i < facesIds.size() - 2; i += 3) {
                ...
                for (std::vector<Vector3f> clipped : Rendering::clipPolygon(faceVerts, m_engineState.nearClipPlane))
                {
                    ...
                    m_drawList.push_back(RenderFace{
                        screenPoints,
                        zCoords,
                        lambertianShading(faceNormals, sf::Color::White), // facesColors[(refIdFaces+i)/3]
        ...
        void SoftwareRenderer::prepareFacesToDraw() {
            ...
            for (const auto& m : m_scene->m_meshes) {
                ...
                facesIds = m->getFacesIndices();
                normalsIds = m->getNormalsIndices();
                facesColors = m->getFacesColors();

                workerPrepareFacesLoop(refIdFaces, refIdNormals);
